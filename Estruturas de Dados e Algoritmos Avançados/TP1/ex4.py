...
print("\n===== Teste 1 - Remoções Sucessivas Até Heap Vazia =====\n")

heap = BinaryHeap()
for v in [4, 10, 3, 5, 1, 8, 2]:
    heap.insert(v)

heap.trocas.clear()

print(f"Estado inicial: {heap._dados}\n")
ordem_remocao: List[int] = []

while not heap.esta_vazia():
    trocas_antes = len(heap.trocas)
    maximo = heap.extract_max()
    ordem_remocao.append(maximo)
    trocas_operacao = heap.trocas[trocas_antes:]

    print(f"=> extract_max() -> {maximo}")
    print(f"=> Heap após remoção: {heap._dados}")

    if trocas_operacao:
        print(f"=> Trocas do sift-down ({len(trocas_operacao)}):")
        for troca in trocas_operacao:
            print(f"  * Índice {troca[0]} (valor {troca[1]}) <-> Índice {troca[2]} (valor {troca[3]})")
    else:
        print(f"=> Trocas do sift-down: nenhuma")

    print()

print(f"Ordem de remoção (deve ser decrescente): {ordem_remocao}")


print("\n\n===== Teste 2 - Robustez: extract_max em Heap Vazia =====\n")

try:
    heap_vazia = BinaryHeap()
    heap_vazia.extract_max()
