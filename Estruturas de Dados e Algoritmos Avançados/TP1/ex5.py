...
print("\n===== Teste 1 - Busca Linear com contains =====\n")

heap_busca = BinaryHeap()

for v in [15, 10, 12, 4, 7, 9, 3]:
    heap_busca.insert(v)

print(f"Heap atual: {heap_busca._dados}\n")

casos = [15, 7, 3, 99, 0]

for valor in casos:
    resultado = heap_busca.contains(valor)
    status = "encontrado" if resultado else "não encontrado"
    print(f"=> contains({valor}) -> {str(resultado)} ({status})")
print()
