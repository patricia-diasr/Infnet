...
print("\n===== Teste 1 - Remoção de Elemento Arbitrário (delete) =====\n")

heap = BinaryHeap()
for v in [15, 10, 12, 4, 7, 9, 3]:
    heap.insert(v)
...
for descricao, valor in casos_delete:
    print(f"Antes: {heap._dados}")
    heap.delete(valor)
    trocas_op = heap.trocas[:]
    heap.trocas.clear()
    print(f"=> delete({valor}) [{descricao}] -> {heap._dados}")

    if trocas_op:
        for troca in trocas_op:
            print(f"=> Troca: índice {troca[0]} (valor {troca[1]}) <-> Índice {troca[2]} (valor {troca[3]})")
    else:
        print(f"=> Sem trocas")
    print()


print("\n===== Teste 2 - Robustez: delete com valores inexistentes e heap vazia =====\n")

try:
    heap.delete(99)
except HeapValueError as e:
    print(f"=> Sucesso ao capturar erro (valor inexistente): {e}")

try:
    heap_vazia = BinaryHeap()
    heap_vazia.delete(1)
except HeapUnderflowError as e:
    print(f"=> Sucesso ao capturar erro (heap vazia): {e}\n")